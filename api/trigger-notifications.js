...
export default async function handler(req, res) {
  ...
  try {
    ...
    let reportData = null;
    try {
      const reportRes = await fetch(REPORT_FN_URL, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${SERVICE_KEY}`,
      ...
      console.log('[trigger-notifications] Report dipendenti:', reportData);
    } catch (err) {
      console.warn('[trigger-notifications] Report error (non bloccante):', err.message);
    }

    console.log('[trigger-notifications] Completato:', data);
    return res.status(200).json({ ...data, report: reportData });
  } catch (err) {
